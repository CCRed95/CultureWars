...
{
	public class InternetArchiveQueryBuilder
		: IInternetArchiveQueryBuilder,
			IQueryBuilder
	{
		...
		public static IInternetArchiveQueryBuilder Builder
		{
			get => new InternetArchiveQueryBuilder();
		}


		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithUploader(
			string uploader)
		{
			if (_uploader != null)
				throw new InvalidOperationException(
					$"The uploader cannot be set to {uploader.Quote()} because the instance of " +
					$"{nameof(InternetArchiveQueryBuilder).SQuote()} already has the uploader {_uploader.Quote()}.");

			_uploader = uploader;
			return this;
		}

		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithSubject(
			string subject)
		{
			if (_subject != null)
				throw new InvalidOperationException(
					$"The subject cannot be set to {subject.Quote()} because the instance of " +
					$"{nameof(InternetArchiveQueryBuilder).SQuote()} already has the subject {_subject.Quote()}.");

			_subject = subject;
			return this;
		}

		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithFields(
			params IQueryField[] fields)
		{
			if (_fields != null)
				throw new InvalidOperationException(
					$"The fields cannot be set to {fields.Aggregate("", (s, i) => s += i.ToString().ToLower()).Quote()} " +
					$"because the instance of {nameof(InternetArchiveQueryBuilder).SQuote()} already has the fields " +
		...
		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithSort(
			...
			IASortDirection sortDirection)
		{
			if (_sortField != null)
				throw new InvalidOperationException(
					...
					$"instance of {nameof(InternetArchiveQueryBuilder).SQuote()} already has the sortField " +
			...
			if (_sortDirection != null)
				throw new InvalidOperationException(
					...
					$"instance of {nameof(InternetArchiveQueryBuilder).SQuote()} already has the sortDirection " +
		...
		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithRows(
			uint rowCount)
		{
			if (_rowCount != null)
				throw new InvalidOperationException(
					$"The rowCount cannot be set to {rowCount.ToString().SQuote()} because the instance of " +
					$"{nameof(InternetArchiveQueryBuilder).SQuote()} already has the rowCount " +
		...
		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.OnPageNumber(
			uint pageNumber)
		{
			if (_onPageNumber != null)
				throw new InvalidOperationException(
					$"The pageNumber cannot be set to {pageNumber.ToString().SQuote()} because the instance of " +
					$"{nameof(InternetArchiveQueryBuilder).SQuote()} already has the pageNumber " +
		...
		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithOutputKind(
			APIDataOutputKind dataOutputKind)
		{
			if (_dataOutputKind != null)
				throw new InvalidOperationException(
					...
					$"because the instance of {nameof(InternetArchiveQueryBuilder).SQuote()} already has the " +
		...
		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithCallback(
			string callback)
		{
			if (_callback != null)
				throw new InvalidOperationException(
					$"The callback cannot be set to {callback.Quote()} because the instance of " +
					$"{nameof(InternetArchiveQueryBuilder).SQuote()} already has the callback {_callback.Quote()}.");

			_callback = callback;
			return this;
		}

		InternetArchiveQueryBuilder IInternetArchiveQueryBuilder.WithShouldSave(
			bool shouldSave)
		{
			if (_shouldSave != null)
				throw new InvalidOperationException(
					$"The shouldSave cannot be set to {shouldSave.ToString().SQuote()} because the instance of " +
					$"{nameof(InternetArchiveQueryBuilder).SQuote()} already has the shouldSave " +
