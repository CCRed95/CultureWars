...
namespace CultureWars.Core.FluentConsole.Colorization.Complex
{
	...
	{
		public List<StyleClass<T>> GenerateGradient<T>(
			IEnumerable<T> input,
			Color startColor,
			Color endColor,
			int maxColorsInGradient)
		{
			...
			bool ShouldChangeColor(int index, int progress, T current, T previous) =>
				(progress > numberOfGrades - 1 && !current.Equals(previous) || IsFirstRun(index));
